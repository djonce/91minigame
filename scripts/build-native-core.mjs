...
await build({
  ...
  plugins: [spritePatch, memoryPatch],
  footer: { js: ['NES', 'Controller', 'sha256', 'directionAt', 'InputSources'].map(name => `module.exports.${name} = NativeNesCore.${name};`).join('\n') },
  legalComments: 'inline', banner: { js: '/* JSNES 2.1.0 (Apache-2.0), modified 2026-09-20: sprites + bulk memory (scripts/jsnes-{sprite,memory}-patch.mjs). @noble/hashes 1.8.0 (MIT). See adjacent licenses. */' },
});
...
console.log('Native NES core bundled (JSNES 2.1.0 + sprites1 + memory1).');
