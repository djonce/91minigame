...
## 2026-09-18 固定摇杆与横屏 A/B 间距

本次只调整手柄交互与布局，不做性能优化，不修改模拟器核心、帧率、声音处理、联机协议或存档格式。

- 默认使用固定八方向摇杆；标准底座 144px、大号 156px，窄屏按现有布局收紧。摇杆头直径为底座的 40%，最大偏移为底座直径的 27%，拖出底座后仍通过指针捕获接收方向，松手立即回中。
- 中心死区为底座半径的 20%；相邻方向边界增加 5° 切换容差，减少手指轻微抖动造成的反复换向。输出仍是 NES 数字方向键，斜向同时发送两个方向。
- 支持方向+A+B 多指操作；抬起、触控取消、丢失捕获、打开菜单、失焦和旋转时释放输入并复位摇杆。保留键盘和无障碍方向按钮。
- 单机菜单和联机房间面板均可切换固定摇杆／十字键、大小和左右布局；偏好保存在同源 localStorage。旧偏好缺少方向控制字段时默认摇杆，其余已保存设置保留。
- 横屏采用 A 右上、B 左下的斜向布局，圆形边缘间距约 28 CSS px，矩形触控区域也不重叠。通过使用纵向空间维持原有按钮直径和游戏画面可用宽度；竖屏 A/B 排列保留。
- 旋转回竖屏时使用根视口边界，修复旧页面宽度使移动浏览器意外缩小整个页面的问题；游戏画面继续保持 4:3。

验证：19 项单元测试通过；Chrome/WebKit 触控套件 9 项通过、1 项跳过（WebKit 不支持该套件使用的 CDP 多指注入）。覆盖八方向拖动、中心死区、拖出底座释放、暂停回中、偏好重载与单机／联机共用、连点不缩放，以及四组横屏尺寸×两种按键大小×左右布局下的间距与边界。横屏切回 390×740 竖屏后缩放恢复为 1，页面宽度为 390。

另有 3 项真实 ROM 回归通过：方向+A+B 多指与键盘部分释放、8 组手机视口、双人＋观战的输入同步与暂停重连。生产构建通过。这些是桌面 Chrome/WebKit 的移动视口验证，不代替 iOS 微信真机手感验收。

截图：[横屏](screenshots/joystick-landscape.png)、[竖屏](screenshots/joystick-portrait.png)。发布版本和在线验收见[部署记录](deployment.md)。
