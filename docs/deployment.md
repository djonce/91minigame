...
部署日期：2026-09-15。部署授权：通过 `ssh aliyun` 将后台服务部署到 `minigames.19ba.cn`。

## 部署内容与当前状态

已部署游戏目录 API、受控 ROM 下载、H5 游戏库与播放器、EmulatorJS 4.2.3 固定运行资源。游戏模拟运行在用户浏览器，服务器负责分发资源。服务器 API 当前只读，没有网页管理后台、账号登录或云存档。

当前发布：`20260915-105326`；Docker 容器健康检查通过。北京时间 19:21:59，Caddy 自动重试成功取得正式证书；19:22 已通过服务器与本机的 HTTPS 请求校验，可访问 [线上游戏库](https://minigames.19ba.cn)。公网浏览器回归结果见下文。
...
| 部署根目录 | `/opt/minigames` |
| 当前代码目录 | `/opt/minigames/current`，指向 `releases/20260915-105326` |
| Compose 项目 / 容器 | `minigames` / `minigames-app` |
| 镜像 | `minigames:20260915-105326` |
...
| 证书 | Let's Encrypt YE1，CN=minigames.19ba.cn；到期 2026-12-14 10:23:24 UTC，Caddy 自动续期 |
...
Docker 使用按摘要固定的官方 Node 24 镜像，服务器不需要执行 npm 安装。镜像构建时再次验证 46 个运行资源的 SHA-256。应用只读运行，设置 512 MiB 内存、1 CPU、100 进程上限；日志每份最多 10 MiB、保留 3 份。配置了重启策略与容器健康检查；健康检查失败本身不会自动重启进程。
...
19:23（北京时间），正式域名 `https://minigames.19ba.cn` 的完整 Chrome 回归通过，用例约 31.3 秒：原始 ROM 摘要一致、核心进入实际关卡并运行超过 1,200 帧、音频上下文正常、方向与 A 键组合可用、暂停停止帧推进、存档导出和恢复摘要一致、重新进入后仍能读取存档、390px 布局无横向溢出。未产生页面脚本异常，游戏资源请求全部同源，开发源码与数据目录均返回 404。Google 公共 DNS 也已返回 `120.55.88.226`。
