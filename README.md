...
2026-09-18 手机界面优化已随版本 `20260918-111911` 部署上线，验收与真机待测项见 [手机界面优化记录](docs/mobile-ui-implementation.md)。iOS 游戏区防误缩放与菜单防选中修复已随 `20260918-120949` 上线，退出当前游戏页后重新进入可加载新资源。小程序播放器页已配置自动横竖屏，需要重新编译后生效。

## 好友联机（测试版）

游戏库新增「好友联机」，或打开[线上联机入口](https://minigames.19ba.cn/netplay.html?game=nes-chise-yaosai)。本地开发可使用[本机入口](http://127.0.0.1:5173/netplay.html?game=nes-chise-yaosai)。房主创建房间，朋友输入房间号加入，全部准备后开始。《赤色要塞》支持 **两人操作＋一人观战**，还需在 ROM 标题菜单选择双人模式。

每个设备独立运行模拟器，网络只同步输入、帧号、校验与恢复快照，不传音视频。玩家切后台或断线会暂停；重连保留原席位，准备后由房主重新同步。房主退出结束本局。当前采用等待输入的帧同步，弱网可能停顿，尚未加入预测回滚。

三人独立操作已经过自制诊断 ROM 的三端验证，真实三人游戏和 iOS 微信多机仍需验收。三人 ROM 登记、实现及测试证据见[联机说明](docs/input-sync-netplay-implementation.md)。此版本已通过 Docker 发布至正式域名，详情见[部署记录](docs/deployment.md)。
